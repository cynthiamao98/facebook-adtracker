(function () {
    ...
    function updateAdLinks(e){
      chrome.storage.local.get({'ads': {}}, function(result) {
        
        var ad_links = result.ads;
        const link = e.querySelector('a[href]:not([href="#"])'); 
        ...
        if (link !== null){
            ...
            if (paths.length >= 3){
                if (!(paths[3] in ad_links)) {
                    ...
                    console.log("FOUND NEW AD: " + paths[3]);
                    //console.log("JSON string is " + JSON.stringify(ad_links));
                    chrome.storage.local.set({'ads': ad_links},function() {
                      console.log('The ADs now are ');
